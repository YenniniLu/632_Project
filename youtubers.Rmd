...
```{r}
pairs(log10(Views) ~ CC + log10(Released) + log10(Length) + log10(Subscribers) + Category + afinn_score, data=df1)
```
...
```{r}
lm1 <- lm(log10(Views) ~ CC + log10(Released) + log10(Length) + log10(Subscribers) + Category + afinn_score, data=df1)
summary(lm1)
```
