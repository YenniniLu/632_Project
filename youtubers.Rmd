...
```{r}
# Fit tree model
t1 <- rpart(Views ~ CC + Released + Category + Length + Subscribers + afinn_score,
            data = df_train,
            method = "anova")
...
```{r}
set.seed(652)
rf1 <- randomForest(Views ~ CC + Released + Category + Length + Subscribers + afinn_score, importance = TRUE, data = df_train)
rf1
```
