...
```{r}
boxcox(Views ~ CC + log10(Released) + log10(Length) + log10(Subscribers) + Category + afinn_score, data=df1,
       lambda = seq(-3, 3, by = 0.05))
```

```{r}
summary(powerTransform(Views ~ 
                         CC + log10(Released) + log10(Length) + log10(Subscribers) + Category + afinn_score, 
                       data=df1))
```
