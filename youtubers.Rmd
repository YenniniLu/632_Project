...
```{r}
pairs(Views ~ CC + Released + log10(Length) + Subscribers + Category + bing_sentiment, data=df)
```


```{r}
boxplot(Views ~ bing_sentiment, data = df)
```
...
```{r}
lm1 <- lm(Views ~ CC + Released + Length + Subscribers + bing_sentiment, data=df)
summary(lm1)
```
...
```{r}
lm2 <- lm(Views ~ CC + Released + log10(Length) + Subscribers + bing_sentiment, data=df)
summary(lm2)
```
