...
```{r}
lm1 <- lm(log10(Views) ~ CC + log10(Released) + log10(Length) + log10(Subscribers) + Category + afinn_score+ afinn_title_score, data=df1)
summary(lm1)
```
...
```{r}
set.seed(652)
rf1 <- randomForest(Views ~ CC + Released + Category + Length + Subscribers + afinn_score + afinn_title_score, importance = TRUE, data = df_train)
rf1
```
